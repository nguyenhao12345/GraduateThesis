...
public final class NoteNameLayer: CATextLayer {
    public init(layerHeight: CGFloat, keyRect: CGRect, noteNumber: Int, label: String) {
        ...
        string = label
        foregroundColor = label.contains("#") ? UIColor.white.cgColor : UIColor.defaultText.cgColor 
//            UIColor.defaultText.cgColor
        backgroundColor = UIColor.clear.cgColor
        font = UIFont.boldSystemFont(ofSize: 0.0)
        fontSize = label.contains("#") ? (keyRect.size.width / 5.0) : (keyRect.size.width / 4.0)
